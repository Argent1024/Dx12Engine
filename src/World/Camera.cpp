...
namespace Game {
	...
	{
		const UINT cbSize = sizeof(DirectX::XMMATRIX);

		ptrGPUMem gpumem = Engine::MemoryAllocator.CreateCommittedBuffer(cbSize);
	...
	void ProjectiveCamera::UseCamera(Graphic::CommandList& commandList, const Transform& ModelTransformation) {
		...
		Transform mvp = m_ViewProjective;
		m_RootCBV->copyData(&mvp);
