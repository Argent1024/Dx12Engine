...
namespace Math
{
	...
	public:
		...
		INLINE explicit Matrix3(ZeroTag) { m_mat[0] = m_mat[1] = m_mat[2] = Vector3(kZero); }
		INLINE explicit Matrix3(IdentityTag) { m_mat[0] = Vector3(kXUnitVec); m_mat[1] = Vector3(kYUnitVec); m_mat[2] = Vector3(kZUnitVec); }
